...
def parse_questions_from_pdf(filepath):
    ...
    def save_current_question():
        ...
        save_current_option()
        print(f"[DEBUG] save_current_question: current_q_no={current_q_no}, text_lines={current_q_text_lines}, options={list(current_options.keys())}")
        if current_q_no and current_q_text_lines:
            ...
            print(f"  [+] Question {current_q_no} appended to questions list. Current size={len(questions)}")
            current_q_no = None
            current_q_text_lines = []
            current_options = {}
    ...
    def save_current_case():
        ...
        print(f"[DEBUG] save_current_case: case_no={current_case_no}, questions_count={len(questions)}")
        if current_case_no:
            ...
            print(f"  [+] Case {current_case_no} appended to cases list. Current size={len(cases)}")
            current_case_no = None
            case_desc_lines = []
            questions = []

    for page_num in range(8, 11): # pages 9-11
        page = doc[page_num]
