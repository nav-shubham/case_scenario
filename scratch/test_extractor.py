...
def should_skip_line(line):
    ...
    if line_upper in ["CASE SCENARIOS", "CASE SCENARIO", "REASON:", "REASON", "REASON :"]:
        return True
...
def extract_answers(filepath):
    ...
    def save_current():
        nonlocal current_case, current_q_no, current_opt, current_val, reasoning_lines
        if current_q_no and current_opt:
            reason = " | ".join(reasoning_lines).strip()
            # Replace character artifacts
            reason = reason.replace('`', '₹').replace('\u20b9', '₹')
            val_clean = current_val.replace('`', '₹').replace('\u20b9', '₹').strip()
...
if __name__ == "__main__":
    ...
    # Use to_string to avoid encoding errors or write to file
    with open("scratch/test_output.txt", "w", encoding="utf-8") as f:
        f.write(df.head(15).to_string())
    print("[+] Wrote head to scratch/test_output.txt")
